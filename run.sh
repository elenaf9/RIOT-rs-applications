...
get_all_benchmarks() {
    exclude="yield|poll|fib|matrix|spinlocks|counter|critical"
    mapfile -t BENCHMARKS < <(find ./benchmarks/ -name "*bench_*" -type d |  grep -vE $exclude )
    for i in $(seq 4)
    do
    ...
    for i in noop cs atomic hardware
    do
